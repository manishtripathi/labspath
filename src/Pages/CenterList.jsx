...
import { getAllCenters } from "../redux/slices/getDropdownoptionSlice";
import Modal from "../Component/Modal/ModalPopUp";
import { Button } from "react-bootstrap";

export default function CenterList() {
  const dispatch = useDispatch();
  const { allCenters } = useSelector((state) => state.dropDownOptions);
  ...
  useEffect(() => {
    if (allCenters?.length > 0) {
      console.log("Using cached data:", allCenters);
    } else {
